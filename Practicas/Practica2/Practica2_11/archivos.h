...
int crearArchivo2(archivo *archivo,char *pathTemp);    // creamos un archivo en una ruta especificada
...
int leerDirectorio(char *pathTemp);                  // esta funcion nos permitira leer los arvhivos uno a uno.
char *obtenerRuta(char *path,char *nombre);          // Obtenemos la ruta del archivo
int copiarArchivo(char *pathTemp ,char * nombre, char *pathCpy);   // copiar archivo a ub directorio 
void listarArchivos(char *pathTemp);                  // manera INCORRECTA DE MANDAR LALMADAS AL SISTEMA
int leerArchivo(archivo *archivo);                   // leer el contenido del archivo mandado
/*
